...
export default function Marquee() {
  return (
    <div className="relative py-6 sm:py-10 overflow-hidden border-y border-white/[0.03] bg-[var(--bg-secondary)]/60">
      {/* Fade edges */}
      ...
      <div className="mb-4">
        <div className="flex animate-marquee-left whitespace-nowrap">
          {[...topItems, ...topItems, ...topItems, ...topItems].map((item, i) => (
            <span
              key={`t-${i}`}
              className="flex items-center gap-3 sm:gap-5 mx-4 sm:mx-6 text-sm sm:text-lg font-[var(--font-syne)] font-bold text-white/[0.06] hover:text-white/20 transition-colors duration-500 select-none"
            >
              {item}
      ...
      <div>
        <div className="flex animate-marquee-right whitespace-nowrap">
          {[...bottomItems, ...bottomItems, ...bottomItems, ...bottomItems].map((item, i) => (
            <span
              key={`b-${i}`}
              className="flex items-center gap-3 sm:gap-5 mx-4 sm:mx-6 text-sm sm:text-lg font-[var(--font-syne)] font-bold text-white/[0.04] hover:text-white/15 transition-colors duration-500 select-none"
            >
              {item}
